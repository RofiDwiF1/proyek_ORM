...
<div class="max-w-7xl mx-auto p-4">
    ...
    <form action="{{ route('employees.update', $employee->id) }}" method="POST" class="bg-white p-6 rounded shadow-md">
        @csrf
        @method('PUT')
        <div class="mb-4">
            <label for="name" class="block text-gray-700 mb-2">Name</label>
            <input type="text" name="name" id="name" value="{{ $employee->name }}" class="w-full border rounded px-3 py-2" required>
        </div>
        ...
        <div class="mb-4">
            ...
            <div class="grid grid-cols-3 gap-2">
                @foreach ($skills as $skill)
                    <div>
                        <input type="checkbox" name="skills[]" id="skill_{{ $skill->id }}" 
                               value="{{ $skill->id }}" {{ $employee->skills->contains($skill->id) ? 'checked' : '' }}>
